<div class="text-end">
  ...
  <br />
  <small>Last <span class="fw-semibold text-warning">Sales</span> refresh: <span id="sales-log"></span></small>
  <br />
    ...
    $(document).ready(function() {
      ...
      RefreshLog.fetchLogTimestamps = function() {
        $.get("{{ route('inventory.log') }}", function(response) {
          ...
          $('#production-log, #sales-log, #procurement-log').html(
      ...
      RefreshLog.submitRefreshInventoryView = function($blockEl) {
        ...
        $.post("{{ route('inventory.refresh') }}", function(response) {
          ...
          $('#production-log, #sales-log').html(`<span class="text-danger fw-semibold">ERROR</span>`);
          showHideBlockUI(false, $blockEl);
        });
      }
      ...
      function setLogTimestamps(response) {
        if (response.success) {
          ...
          $('#sales-log').text(
            response.data.sales_log ? dayjs(response.data.sales_log).format('DD-MMM-YYYY HH:mm:ss') : '-');
          $('#procurement-log').text(
            response.data.procurement_log ? dayjs(response.data.procurement_log).format('DD-MMM-YYYY HH:mm:ss') :
            '-');
        } else {
          $('#production-log #sales-log #procurement-log').text('-');
        }
      }
    });
  </script>
